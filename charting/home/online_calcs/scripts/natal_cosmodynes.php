...
if (isset($_POST['submitted'])) {
    ...
    // get ID1
    $last_id = mysqlSafeEscapeString($conn, $_POST["id1"]);
    ...
    $secs = $whole_seconds;// assign $secs here becase it is used below and not defidne anywhere esle
...
//done
    ...
    $result = GetNatalCosmodynes($pl_name, $longitude, $declination, $house_pos, (isset($unknown_bt)?$unknown_bt:null), $MRs, (isset($$name)?$$name:null), $planet_power, $planet_harmony, $sign_power, $sign_harmony, $house_power, $house_harmony, $totals);
...
//display the results - first reduce to 2 decimal places
    ...
    for ($xx = 0; $xx <= 11; $xx++) {
        ...
        $totals = fill1DArray($totals,6);
        if ($xx <= 6) {
            $xxi = $xx-1;
            if ($xxi > 0) {
                $totals[$xx] = sprintf('%.2f', $totals[$xx]);
            }
        }
    }
...
Function GetMutualReceptions_natal($longitude, &$MRs) {
    ...
    for ($y = 0; $y <= 9; $y++) {
        /* preassign so we don;t get crashed on missing offsets*/
        if (!isset($MR[$y])) {
            $MR[$y]=0;
        }
        if (!isset($MRs[$y])) {
...
//find total planet power
    $PlanetHousePower = fill1DArray( (isset($PlanetHousePower)?$PlanetHousePower:array()) ,11);
    $PlanetTotalPower = fill1DArray( (isset($PlanetTotalPower)?$PlanetTotalPower:array()) ,11);
    $aspect_power = fill1DArray( (isset($aspect_power)?$aspect_power:array()) ,11);
...
//calculate sign power
    ...
    if ($InterceptedSign[1] == 0) {
        $SignPower[1] = $PlanetTotalPower[4] / 4;
    }
    if ($InterceptedSign[1] == 1) {
        $SignPower[1] = $PlanetTotalPower[4] / 2;
    }
    if ($InterceptedSign[1] == 2) {
        $SignPower[1] = $PlanetTotalPower[4];
    }
    if ($InterceptedSign[1] == 3) {
        $SignPower[1] = $PlanetTotalPower[4] * 2;
    }
    if ($InterceptedSign[2] == 0) {
        $SignPower[2] = $PlanetTotalPower[3] / 4;
    }
    if ($InterceptedSign[2] == 1) {
        $SignPower[2] = $PlanetTotalPower[3] / 2;
    }
    if ($InterceptedSign[2] == 2) {
        $SignPower[2] = $PlanetTotalPower[3];
    }
    if ($InterceptedSign[2] == 3) {
        $SignPower[2] = $PlanetTotalPower[3] * 2;
    }
    if ($InterceptedSign[3] == 0) {
        $SignPower[3] = $PlanetTotalPower[2] / 4;
    }
    if ($InterceptedSign[3] == 1) {
        $SignPower[3] = $PlanetTotalPower[2] / 2;
    }
    if ($InterceptedSign[3] == 2) {
        $SignPower[3] = $PlanetTotalPower[2];
    }
    if ($InterceptedSign[3] == 3) {
        $SignPower[3] = $PlanetTotalPower[2] * 2;
    }
    if ($InterceptedSign[4] == 0) {
        $SignPower[4] = $PlanetTotalPower[1] / 4;
    }
    if ($InterceptedSign[4] == 1) {
        $SignPower[4] = $PlanetTotalPower[1] / 2;
    }
    if ($InterceptedSign[4] == 2) {
        $SignPower[4] = $PlanetTotalPower[1];
    }
    if ($InterceptedSign[4] == 3) {
        $SignPower[4] = $PlanetTotalPower[1] * 2;
    }
    if ($InterceptedSign[5] == 0) {
        $SignPower[5] = $PlanetTotalPower[0] / 4;
    }
    if ($InterceptedSign[5] == 1) {
        $SignPower[5] = $PlanetTotalPower[0] / 2;
    }
    if ($InterceptedSign[5] == 2) {
        $SignPower[5] = $PlanetTotalPower[0];
    }
    if ($InterceptedSign[5] == 3) {
        $SignPower[5] = $PlanetTotalPower[0] * 2;
    }
    if ($InterceptedSign[6] == 0) {
        $SignPower[6] = $PlanetTotalPower[2] / 4;
    }
    if ($InterceptedSign[6] == 1) {
        $SignPower[6] = $PlanetTotalPower[2] / 2;
    }
    if ($InterceptedSign[6] == 2) {
        $SignPower[6] = $PlanetTotalPower[2];
    }
    if ($InterceptedSign[6] == 3) {
        $SignPower[6] = $PlanetTotalPower[2] * 2;
    }
    if ($InterceptedSign[7] == 0) {
        $SignPower[7] = $PlanetTotalPower[3] / 4;
    }
    if ($InterceptedSign[7] == 1) {
        $SignPower[7] = $PlanetTotalPower[3] / 2;
    }
    if ($InterceptedSign[7] == 2) {
        $SignPower[7] = $PlanetTotalPower[3];
    }
    if ($InterceptedSign[7] == 3) {
        $SignPower[7] = $PlanetTotalPower[3] * 2;
    }
    if ($InterceptedSign[8] == 0) {
        $SignPower[8] = (($PlanetTotalPower[4] + $PlanetTotalPower[9]) / 2) / 4;
    }
    if ($InterceptedSign[8] == 1) {
        $SignPower[8] = (($PlanetTotalPower[4] + $PlanetTotalPower[9]) / 2) / 2;
    }
    if ($InterceptedSign[8] == 2) {
        $SignPower[8] = (($PlanetTotalPower[4] + $PlanetTotalPower[9]) / 2);
    }
    if ($InterceptedSign[8] == 3) {
        $SignPower[8] = (($PlanetTotalPower[4] + $PlanetTotalPower[9]) / 2) * 2;
    }
    if ($InterceptedSign[9] == 0) {
        $SignPower[9] = $PlanetTotalPower[5] / 4;
    }
    if ($InterceptedSign[9] == 1) {
        $SignPower[9] = $PlanetTotalPower[5] / 2;
    }
    if ($InterceptedSign[9] == 2) {
        $SignPower[9] = $PlanetTotalPower[5];
    }
    if ($InterceptedSign[9] == 3) {
        $SignPower[9] = $PlanetTotalPower[5] * 2;
    }
    if ($InterceptedSign[10] == 0) {
        $SignPower[10] = $PlanetTotalPower[6] / 4;
    }
    if ($InterceptedSign[10] == 1) {
        $SignPower[10] = $PlanetTotalPower[6] / 2;
    }
    if ($InterceptedSign[10] == 2) {
        $SignPower[10] = $PlanetTotalPower[6];
    }
    if ($InterceptedSign[10] == 3) {
        $SignPower[10] = $PlanetTotalPower[6] * 2;
    }
    if ($InterceptedSign[11] == 0) {
        $SignPower[11] = (($PlanetTotalPower[6] + $PlanetTotalPower[7]) / 2) / 4;
    }
    if ($InterceptedSign[11] == 1) {
        $SignPower[11] = (($PlanetTotalPower[6] + $PlanetTotalPower[7]) / 2) / 2;
    }
    if ($InterceptedSign[11] == 2) {
        $SignPower[11] = (($PlanetTotalPower[6] + $PlanetTotalPower[7]) / 2);
    }
    if ($InterceptedSign[11] == 3) {
        $SignPower[11] = (($PlanetTotalPower[6] + $PlanetTotalPower[7]) / 2) * 2;
    }
    if ($InterceptedSign[12] == 0) {
        $SignPower[12] = (($PlanetTotalPower[5] + $PlanetTotalPower[8]) / 2) / 4;
    }
    if ($InterceptedSign[12] == 1) {
        $SignPower[12] = (($PlanetTotalPower[5] + $PlanetTotalPower[8]) / 2) / 2;
    }
    if ($InterceptedSign[12] == 2) {
        $SignPower[12] = (($PlanetTotalPower[5] + $PlanetTotalPower[8]) / 2);
    }
    if ($InterceptedSign[12] == 3) {
        $SignPower[12] = (($PlanetTotalPower[5] + $PlanetTotalPower[8]) / 2) * 2;
    }

    //find unoccupied sign harmony
    if ($InterceptedSign[1] == 0) {
        $SignHarmony[1] = $harmony[4] / 4;
    }
    if ($InterceptedSign[1] == 1) {
        $SignHarmony[1] = $harmony[4] / 2;
    }
    if ($InterceptedSign[1] == 2) {
        $SignHarmony[1] = $harmony[4];
    }
    if ($InterceptedSign[1] == 3) {
        $SignHarmony[1] = $harmony[4] * 2;
    }
    if ($InterceptedSign[2] == 0) {
        $SignHarmony[2] = $harmony[3] / 4;
    }
    if ($InterceptedSign[2] == 1) {
        $SignHarmony[2] = $harmony[3] / 2;
    }
    if ($InterceptedSign[2] == 2) {
        $SignHarmony[2] = $harmony[3];
    }
    if ($InterceptedSign[2] == 3) {
        $SignHarmony[2] = $harmony[3] * 2;
    }
    if ($InterceptedSign[3] == 0) {
        $SignHarmony[3] = $harmony[2] / 4;
    }
    if ($InterceptedSign[3] == 1) {
        $SignHarmony[3] = $harmony[2] / 2;
    }
    if ($InterceptedSign[3] == 2) {
        $SignHarmony[3] = $harmony[2];
    }
    if ($InterceptedSign[3] == 3) {
        $SignHarmony[3] = $harmony[2] * 2;
    }
    if ($InterceptedSign[4] == 0) {
        $SignHarmony[4] = $harmony[1] / 4;
    }
    if ($InterceptedSign[4] == 1) {
        $SignHarmony[4] = $harmony[1] / 2;
    }
    if ($InterceptedSign[4] == 2) {
        $SignHarmony[4] = $harmony[1];
    }
    if ($InterceptedSign[4] == 3) {
        $SignHarmony[4] = $harmony[1] * 2;
    }
    if ($InterceptedSign[5] == 0) {
        $SignHarmony[5] = $harmony[0] / 4;
    }
    if ($InterceptedSign[5] == 1) {
        $SignHarmony[5] = $harmony[0] / 2;
    }
    if ($InterceptedSign[5] == 2) {
        $SignHarmony[5] = $harmony[0];
    }
    if ($InterceptedSign[5] == 3) {
        $SignHarmony[5] = $harmony[0] * 2;
    }
    if ($InterceptedSign[6] == 0) {
        $SignHarmony[6] = $harmony[2] / 4;
    }
    if ($InterceptedSign[6] == 1) {
        $SignHarmony[6] = $harmony[2] / 2;
    }
    if ($InterceptedSign[6] == 2) {
        $SignHarmony[6] = $harmony[2];
    }
    if ($InterceptedSign[6] == 3) {
        $SignHarmony[6] = $harmony[2] * 2;
    }
    if ($InterceptedSign[7] == 0) {
        $SignHarmony[7] = $harmony[3] / 4;
    }
    if ($InterceptedSign[7] == 1) {
        $SignHarmony[7] = $harmony[3] / 2;
    }
    if ($InterceptedSign[7] == 2) {
        $SignHarmony[7] = $harmony[3];
    }
    if ($InterceptedSign[7] == 3) {
        $SignHarmony[7] = $harmony[3] * 2;
    }
    if ($InterceptedSign[8] == 0) {
        $SignHarmony[8] = (($harmony[4] + $harmony[9]) / 2) / 4;
    }
    if ($InterceptedSign[8] == 1) {
        $SignHarmony[8] = (($harmony[4] + $harmony[9]) / 2) / 2;
    }
    if ($InterceptedSign[8] == 2) {
        $SignHarmony[8] = (($harmony[4] + $harmony[9]) / 2);
    }
    if ($InterceptedSign[8] == 3) {
        $SignHarmony[8] = (($harmony[4] + $harmony[9]) / 2) * 2;
    }
    if ($InterceptedSign[9] == 0) {
        $SignHarmony[9] = $harmony[5] / 4;
    }
    if ($InterceptedSign[9] == 1) {
        $SignHarmony[9] = $harmony[5] / 2;
    }
    if ($InterceptedSign[9] == 2) {
        $SignHarmony[9] = $harmony[5];
    }
    if ($InterceptedSign[9] == 3) {
        $SignHarmony[9] = $harmony[5] * 2;
    }
    if ($InterceptedSign[10] == 0) {
        $SignHarmony[10] = $harmony[6] / 4;
    }
    if ($InterceptedSign[10] == 1) {
        $SignHarmony[10] = $harmony[6] / 2;
    }
    if ($InterceptedSign[10] == 2) {
        $SignHarmony[10] = $harmony[6];
    }
    if ($InterceptedSign[10] == 3) {
        $SignHarmony[10] = $harmony[6] * 2;
    }
    if ($InterceptedSign[11] == 0) {
        $SignHarmony[11] = (($harmony[6] + $harmony[7]) / 2) / 4;
    }
    if ($InterceptedSign[11] == 1) {
        $SignHarmony[11] = (($harmony[6] + $harmony[7]) / 2) / 2;
    }
    if ($InterceptedSign[11] == 2) {
        $SignHarmony[11] = (($harmony[6] + $harmony[7]) / 2);
    }
    if ($InterceptedSign[11] == 3) {
        $SignHarmony[11] = (($harmony[6] + $harmony[7]) / 2) * 2;
    }
    if ($InterceptedSign[12] == 0) {
        $SignHarmony[12] = (($harmony[5] + $harmony[8]) / 2) / 4;
    }
    if ($InterceptedSign[12] == 1) {
        $SignHarmony[12] = (($harmony[5] + $harmony[8]) / 2) / 2;
    }
    if ($InterceptedSign[12] == 2) {
        $SignHarmony[12] = (($harmony[5] + $harmony[8]) / 2);
    }
    if ($InterceptedSign[12] == 3) {
        $SignHarmony[12] = (($harmony[5] + $harmony[8]) / 2) * 2;
    }
    ...
    if ($InterceptedSign[1] == 0) {
        $SignDiscord[1] = $discord[4] / 4;
    }
    if ($InterceptedSign[1] == 1) {
        $SignDiscord[1] = $discord[4] / 2;
    }
    if ($InterceptedSign[1] == 2) {
        $SignDiscord[1] = $discord[4];
    }
    if ($InterceptedSign[1] == 3) {
        $SignDiscord[1] = $discord[4] * 2;
    }
    if ($InterceptedSign[2] == 0) {
        $SignDiscord[2] = $discord[3] / 4;
    }
    if ($InterceptedSign[2] == 1) {
        $SignDiscord[2] = $discord[3] / 2;
    }
    if ($InterceptedSign[2] == 2) {
        $SignDiscord[2] = $discord[3];
    }
    if ($InterceptedSign[2] == 3) {
        $SignDiscord[2] = $discord[3] * 2;
    }
    if ($InterceptedSign[3] == 0) {
        $SignDiscord[3] = $discord[2] / 4;
    }
    if ($InterceptedSign[3] == 1) {
        $SignDiscord[3] = $discord[2] / 2;
    }
    if ($InterceptedSign[3] == 2) {
        $SignDiscord[3] = $discord[2];
    }
    if ($InterceptedSign[3] == 3) {
        $SignDiscord[3] = $discord[2] * 2;
    }
    if ($InterceptedSign[4] == 0) {
        $SignDiscord[4] = $discord[1] / 4;
    }
    if ($InterceptedSign[4] == 1) {
        $SignDiscord[4] = $discord[1] / 2;
    }
    if ($InterceptedSign[4] == 2) {
        $SignDiscord[4] = $discord[1];
    }
    if ($InterceptedSign[4] == 3) {
        $SignDiscord[4] = $discord[1] * 2;
    }
    if ($InterceptedSign[5] == 0) {
        $SignDiscord[5] = $discord[0] / 4;
    }
    if ($InterceptedSign[5] == 1) {
        $SignDiscord[5] = $discord[0] / 2;
    }
    if ($InterceptedSign[5] == 2) {
        $SignDiscord[5] = $discord[0];
    }
    if ($InterceptedSign[5] == 3) {
        $SignDiscord[5] = $discord[0] * 2;
    }
    if ($InterceptedSign[6] == 0) {
        $SignDiscord[6] = $discord[2] / 4;
    }
    if ($InterceptedSign[6] == 1) {
        $SignDiscord[6] = $discord[2] / 2;
    }
    if ($InterceptedSign[6] == 2) {
        $SignDiscord[6] = $discord[2];
    }
    if ($InterceptedSign[6] == 3) {
        $SignDiscord[6] = $discord[2] * 2;
    }
    if ($InterceptedSign[7] == 0) {
        $SignDiscord[7] = $discord[3] / 4;
    }
    if ($InterceptedSign[7] == 1) {
        $SignDiscord[7] = $discord[3] / 2;
    }
    if ($InterceptedSign[7] == 2) {
        $SignDiscord[7] = $discord[3];
    }
    if ($InterceptedSign[7] == 3) {
        $SignDiscord[7] = $discord[3] * 2;
    }
    if ($InterceptedSign[8] == 0) {
        $SignDiscord[8] = (($discord[4] + $discord[9]) / 2) / 4;
    }
    if ($InterceptedSign[8] == 1) {
        $SignDiscord[8] = (($discord[4] + $discord[9]) / 2) / 2;
    }
    if ($InterceptedSign[8] == 2) {
        $SignDiscord[8] = (($discord[4] + $discord[9]) / 2);
    }
    if ($InterceptedSign[8] == 3) {
        $SignDiscord[8] = (($discord[4] + $discord[9]) / 2) * 2;
    }
    if ($InterceptedSign[9] == 0) {
        $SignDiscord[9] = $discord[5] / 4;
    }
    if ($InterceptedSign[9] == 1) {
        $SignDiscord[9] = $discord[5] / 2;
    }
    if ($InterceptedSign[9] == 2) {
        $SignDiscord[9] = $discord[5];
    }
    if ($InterceptedSign[9] == 3) {
        $SignDiscord[9] = $discord[5] * 2;
    }
    if ($InterceptedSign[10] == 0) {
        $SignDiscord[10] = $discord[6] / 4;
    }
    if ($InterceptedSign[10] == 1) {
        $SignDiscord[10] = $discord[6] / 2;
    }
    if ($InterceptedSign[10] == 2) {
        $SignDiscord[10] = $discord[6];
    }
    if ($InterceptedSign[10] == 3) {
        $SignDiscord[10] = $discord[6] * 2;
    }
    if ($InterceptedSign[11] == 0) {
        $SignDiscord[11] = (($discord[6] + $discord[7]) / 2) / 4;
    }
    if ($InterceptedSign[11] == 1) {
        $SignDiscord[11] = (($discord[6] + $discord[7]) / 2) / 2;
    }
    if ($InterceptedSign[11] == 2) {
        $SignDiscord[11] = (($discord[6] + $discord[7]) / 2);
    }
    if ($InterceptedSign[11] == 3) {
        $SignDiscord[11] = (($discord[6] + $discord[7]) / 2) * 2;
    }
    if ($InterceptedSign[12] == 0) {
        $SignDiscord[12] = (($discord[5] + $discord[8]) / 2) / 4;
    }
    if ($InterceptedSign[12] == 1) {
        $SignDiscord[12] = (($discord[5] + $discord[8]) / 2) / 2;
    }
    if ($InterceptedSign[12] == 2) {
        $SignDiscord[12] = (($discord[5] + $discord[8]) / 2);
    }
    if ($InterceptedSign[12] == 3) {
        $SignDiscord[12] = (($discord[5] + $discord[8]) / 2) * 2;
    }
...
//calculate house power
    ...
    for ($x = 1; $x <= 12; $x++) {
        ...
        if ($tmp7 == 1) {
            $HousePower[$x] = $PlanetTotalPower[4] / 2;
        }
        if ($tmp7 == 2) {
            $HousePower[$x] = $PlanetTotalPower[3] / 2;
        }
        if ($tmp7 == 3) {
            $HousePower[$x] = $PlanetTotalPower[2] / 2;
        }
        if ($tmp7 == 4) {
            $HousePower[$x] = $PlanetTotalPower[1] / 2;
        }
        if ($tmp7 == 5) {
            $HousePower[$x] = $PlanetTotalPower[0] / 2;
        }
        if ($tmp7 == 6) {
            $HousePower[$x] = $PlanetTotalPower[2] / 2;
        }
        ...
        if ($tmp7 == 8) {
            $HousePower[$x] = (($PlanetTotalPower[4] + $PlanetTotalPower[9]) / 2) / 2;
        }
        if ($tmp7 == 9) {
            $HousePower[$x] = $PlanetTotalPower[5] / 2;
        }
        if ($tmp7 == 10) {
            $HousePower[$x] = $PlanetTotalPower[6] / 2;
        }
        if ($tmp7 == 11) {
            $HousePower[$x] = (($PlanetTotalPower[6] + $PlanetTotalPower[7]) / 2) / 2;
        }
        if ($tmp7 == 12) {
            $HousePower[$x] = (($PlanetTotalPower[5] + $PlanetTotalPower[8]) / 2) / 2;
        }
...
//CHP1:
            ...
            while ($tmp7 < $tmp8) {
                ...
                if ($t7 == 1) {
                    $HousePower[$x] = $HousePower[$x] + $PlanetTotalPower[4] / 4;
                }
                if ($t7 == 2) {
                    $HousePower[$x] = $HousePower[$x] + $PlanetTotalPower[3] / 4;
                }
                if ($t7 == 3) {
                    $HousePower[$x] = $HousePower[$x] + $PlanetTotalPower[2] / 4;
                }
                if ($t7 == 4) {
                    $HousePower[$x] = $HousePower[$x] + $PlanetTotalPower[1] / 4;
                }
                if ($t7 == 5) {
                    $HousePower[$x] = $HousePower[$x] + $PlanetTotalPower[0] / 4;
                }
                if ($t7 == 6) {
                    $HousePower[$x] = $HousePower[$x] + $PlanetTotalPower[2] / 4;
                }
                ...
                if ($t7 == 8) {
                    $HousePower[$x] = $HousePower[$x] + (($PlanetTotalPower[4] + $PlanetTotalPower[9]) / 2) / 4;
                }
                if ($t7 == 9) {
                    $HousePower[$x] = $HousePower[$x] + $PlanetTotalPower[5] / 4;
                }
                if ($t7 == 10) {
                    $HousePower[$x] = $HousePower[$x] + $PlanetTotalPower[6] / 4;
                }
                if ($t7 == 11) {
                    $HousePower[$x] = $HousePower[$x] + (($PlanetTotalPower[6] + $PlanetTotalPower[7]) / 2) / 4;
                }
                if ($t7 == 12) {
                    $HousePower[$x] = $HousePower[$x] + (($PlanetTotalPower[5] + $PlanetTotalPower[8]) / 2) / 4;
                }
                $tmp7 = $tmp7 + 1;
            }
        }
    ...
    for ($x = 1; $x <= 12; $x++) {
        ...
        //do ruler of sign on house cusp
        if ($tmp7 == 1) {
            $HouseHarmony[$x] = $harmony[4] / 2;
        }
        if ($tmp7 == 2) {
            $HouseHarmony[$x] = $harmony[3] / 2;
        }
        if ($tmp7 == 3) {
            $HouseHarmony[$x] = $harmony[2] / 2;
        }
        if ($tmp7 == 4) {
            $HouseHarmony[$x] = $harmony[1] / 2;
        }
        if ($tmp7 == 5) {
            $HouseHarmony[$x] = $harmony[0] / 2;
        }
        if ($tmp7 == 6) {
            $HouseHarmony[$x] = $harmony[2] / 2;
        }
        ...
        if ($tmp7 == 8) {
            $HouseHarmony[$x] = (($harmony[4] + $harmony[9]) / 2) / 2;
        }
        if ($tmp7 == 9) {
            $HouseHarmony[$x] = $harmony[5] / 2;
        }
        if ($tmp7 == 10) {
            $HouseHarmony[$x] = $harmony[6] / 2;
        }
        if ($tmp7 == 11) {
            $HouseHarmony[$x] = (($harmony[6] + $harmony[7]) / 2) / 2;
        }
        if ($tmp7 == 12) {
            $HouseHarmony[$x] = (($harmony[5] + $harmony[8]) / 2) / 2;
        }
...
//CHP1:
            ...
            while ($tmp7 < $tmp8) {
                ...
                if ($t7 == 1) {
                    $HouseHarmony[$x] = $HouseHarmony[$x] + $harmony[4] / 4;
                }
                if ($t7 == 2) {
                    $HouseHarmony[$x] = $HouseHarmony[$x] + $harmony[3] / 4;
                }
                if ($t7 == 3) {
                    $HouseHarmony[$x] = $HouseHarmony[$x] + $harmony[2] / 4;
                }
                if ($t7 == 4) {
                    $HouseHarmony[$x] = $HouseHarmony[$x] + $harmony[1] / 4;
                }
                if ($t7 == 5) {
                    $HouseHarmony[$x] = $HouseHarmony[$x] + $harmony[0] / 4;
                }
                if ($t7 == 6) {
                    $HouseHarmony[$x] = $HouseHarmony[$x] + $harmony[2] / 4;
                }
                ...
                if ($t7 == 8) {
                    $HouseHarmony[$x] = $HouseHarmony[$x] + (($harmony[4] + $harmony[9]) / 2) / 4;
                }
                if ($t7 == 9) {
                    $HouseHarmony[$x] = $HouseHarmony[$x] + $harmony[5] / 4;
                }
                if ($t7 == 10) {
                    $HouseHarmony[$x] = $HouseHarmony[$x] + $harmony[6] / 4;
                }
                if ($t7 == 11) {
                    $HouseHarmony[$x] = $HouseHarmony[$x] + (($harmony[6] + $harmony[7]) / 2) / 4;
                }
                if ($t7 == 12) {
                    $HouseHarmony[$x] = $HouseHarmony[$x] + (($harmony[5] + $harmony[8]) / 2) / 4;
                }
                $tmp7 = $tmp7 + 1;
            }
        }
    ...
    for ($x = 1; $x <= 12; $x++) {
        ...
        //do ruler of sign on house cusp
        if ($tmp7 == 1) {
            $HouseDiscord[$x] = $discord[4] / 2;
        }
        if ($tmp7 == 2) {
            $HouseDiscord[$x] = $discord[3] / 2;
        }
        if ($tmp7 == 3) {
            $HouseDiscord[$x] = $discord[2] / 2;
        }
        if ($tmp7 == 4) {
            $HouseDiscord[$x] = $discord[1] / 2;
        }
        if ($tmp7 == 5) {
            $HouseDiscord[$x] = $discord[0] / 2;
        }
        if ($tmp7 == 6) {
            $HouseDiscord[$x] = $discord[2] / 2;
        }
        ...
        if ($tmp7 == 8) {
            $HouseDiscord[$x] = (($discord[4] + $discord[9]) / 2) / 2;
        }
        if ($tmp7 == 9) {
            $HouseDiscord[$x] = $discord[5] / 2;
        }
        if ($tmp7 == 10) {
            $HouseDiscord[$x] = $discord[6] / 2;
        }
        if ($tmp7 == 11) {
            $HouseDiscord[$x] = (($discord[6] + $discord[7]) / 2) / 2;
        }
        if ($tmp7 == 12) {
            $HouseDiscord[$x] = (($discord[5] + $discord[8]) / 2) / 2;
        }
...
//CHP1:
            ...
            while ($tmp7 < $tmp8) {
                ...
                if ($t7 == 1) {
                    $HouseDiscord[$x] = $HouseDiscord[$x] + $discord[4] / 4;
                }
                if ($t7 == 2) {
                    $HouseDiscord[$x] = $HouseDiscord[$x] + $discord[3] / 4;
                }
                if ($t7 == 3) {
                    $HouseDiscord[$x] = $HouseDiscord[$x] + $discord[2] / 4;
                }
                if ($t7 == 4) {
                    $HouseDiscord[$x] = $HouseDiscord[$x] + $discord[1] / 4;
                }
                if ($t7 == 5) {
                    $HouseDiscord[$x] = $HouseDiscord[$x] + $discord[0] / 4;
                }
                if ($t7 == 6) {
                    $HouseDiscord[$x] = $HouseDiscord[$x] + $discord[2] / 4;
                }
                ...
                if ($t7 == 8) {
                    $HouseDiscord[$x] = $HouseDiscord[$x] + (($discord[4] + $discord[9]) / 2) / 4;
                }
                if ($t7 == 9) {
                    $HouseDiscord[$x] = $HouseDiscord[$x] + $discord[5] / 4;
                }
                if ($t7 == 10) {
                    $HouseDiscord[$x] = $HouseDiscord[$x] + $discord[6] / 4;
                }
                if ($t7 == 11) {
                    $HouseDiscord[$x] = $HouseDiscord[$x] + (($discord[6] + $discord[7]) / 2) / 4;
                }
                if ($t7 == 12) {
                    $HouseDiscord[$x] = $HouseDiscord[$x] + (($discord[5] + $discord[8]) / 2) / 4;
                }
                $tmp7 = $tmp7 + 1;
            }
        }
...
function fill1DArray($ary,$l,$v=null) {
    if (!isset($ary[$l])) {
        for ($i = 0; $i <= $l; $i++) {
            if (!isset($ary[$i])) {
                $ary[$i] = $v;
