...
$ubt1 = intval(mysqlSafeEscapeString($conn, (isset($_GET["ubt1"]) ? $_GET["l1"] : "")));

$retrograde = mysqlSafeEscapeString($conn, (isset($_GET["rx1"]) ? $_GET["l1"] : ""));

$longitude = $_SESSION['longitude2_HA'];
$hc = $_SESSION['hc2_HA'];
$house_pos = $_SESSION['house_pos2_HA'];

var_dump($house_pos);exit;

$longitude2_MC = $_SESSION['longitude2_MC'];          //"floating" MC

saveSession("harmony_wheel");
...
for ($i = $num_planets - 1; $i >= 0; $i--) {
    ...
    $temp = $house_num;
//    $sort_pos = syncSize($sort_pos,$i,0);
//    $house_pos = syncSize($house_pos,$sort_pos[$i],0);//JWX
    ...
    $planets_done = (isset($planets_done) ? $planets_done : $i);
//    $nopih = syncSize($nopih,$house_num,0);
    if ($nopih[$house_num] - $planets_done >= $how_many_more_can_fit_in_this_house) {
...
//      $sort_pos = syncSize($sort_pos,$i,0);
//      $sort_pos = syncSize($sort_pos,$j,0);
//      $longitude = syncSize($longitude,$sort_pos[$i],0);//JWX
//      $longitude = syncSize($longitude,$sort_pos[$j],0);//JWX
...
//        for($j=0;$j<$lpn;$j++) {
//            $dd[$j] = (isset($sort_pos[$j])?$sort_pos[$j]:0);
//        }
//        $sort_pos = $dd;    
//        for($j=0;$j<$lpn;$j++) {
//            $dd[$j] = (isset($house_pos[$j])?$house_pos[$j]:0);
//        }
//        $house_pos = $dd;    
