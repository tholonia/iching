...
if (isset($_REQUEST['session_name'])) {
    ...
    loadSession("contests");
}

//var_dump($_SESSION);
$sessionName = "contest_wheel";
...
for ($i = 1; $i <= 12; $i = $i + 1) {
    ...
    $angle_to_use = (isset($angle_sum) ? $angle_sum : 0) + ($angle_diff / 2);
    ...
    $angle_sum = (isset($angle_sum) ? $angle_sum : 0) + $angle_diff;    //26 March 2010
}
...
for ($i = $num_planets - 1; $i >= 0; $i--) {
    ...
    // display Rx symbol
    if (isset($retrograde[$sort_pos[$i]])) {
...
for ($i = 0; $i <= LAST_PLANET - 1; $i++) {
    for ($j = $i + 1; $j <= LAST_PLANET; $j++) {
        if ($sort_pos[$i] > SE_SATURN Or $sort_pos[$j] > SE_SATURN) {
            continue;
        }
        if (!isset($moiety[$sort_pos[$i]])) {
            continue;
        }
        $q = -99;
        ...
        for ($k = 0; $k <= 4; $k++) {
            if (isset($exact_aspect_special[$k])) {
...
//find exaltation of this planet's sign
    ...
    if ($exalt >= 0) {
        drawboldtext($im, 14, 0, $left_margin + 135, $top_of_planet_listing + ($cnt * $y_spacing), $black, './HamburgSymbols.ttf', chr($pl_glyph[$exalt]), 0);
    }     //draw planet glyph
...
//find term of this planet's sign
    if ($longitude[$i] >= 0 And $longitude[$i] < 6) {
        $term = SE_JUPITER;
    }
    if ($longitude[$i] >= 6 And $longitude[$i] < 14) {
        $term = SE_VENUS;
    }
    if ($longitude[$i] >= 14 And $longitude[$i] < 21) {
        $term = SE_MERCURY;
    }
    if ($longitude[$i] >= 21 And $longitude[$i] < 26) {
        $term = SE_MARS;
    }
    if ($longitude[$i] >= 26 And $longitude[$i] < 30) {
        $term = SE_SATURN;
    }
    if ($longitude[$i] >= 30 And $longitude[$i] < 38) {
        $term = SE_VENUS;
    }
    if ($longitude[$i] >= 38 And $longitude[$i] < 45) {
        $term = SE_MERCURY;
    }
    if ($longitude[$i] >= 45 And $longitude[$i] < 52) {
        $term = SE_JUPITER;
    }
    if ($longitude[$i] >= 52 And $longitude[$i] < 56) {
        $term = SE_SATURN;
    }
    if ($longitude[$i] >= 56 And $longitude[$i] < 60) {
        $term = SE_MARS;
    }
    if ($longitude[$i] >= 60 And $longitude[$i] < 67) {
        $term = SE_MERCURY;
    }
    if ($longitude[$i] >= 67 And $longitude[$i] < 74) {
        $term = SE_JUPITER;
    }
    if ($longitude[$i] >= 74 And $longitude[$i] < 81) {
        $term = SE_VENUS;
    }
    if ($longitude[$i] >= 81 And $longitude[$i] < 85) {
        $term = SE_SATURN;
    }
    if ($longitude[$i] >= 85 And $longitude[$i] < 90) {
        $term = SE_MARS;
    }
    if ($longitude[$i] >= 90 And $longitude[$i] < 96) {
        $term = SE_MARS;
    }
    if ($longitude[$i] >= 96 And $longitude[$i] < 103) {
        $term = SE_JUPITER;
    }
    if ($longitude[$i] >= 103 And $longitude[$i] < 110) {
        $term = SE_MERCURY;
    }
    if ($longitude[$i] >= 110 And $longitude[$i] < 117) {
        $term = SE_VENUS;
    }
    if ($longitude[$i] >= 117 And $longitude[$i] < 120) {
        $term = SE_SATURN;
    }
    if ($longitude[$i] >= 120 And $longitude[$i] < 126) {
        $term = SE_SATURN;
    }
    if ($longitude[$i] >= 126 And $longitude[$i] < 133) {
        $term = SE_MERCURY;
    }
    if ($longitude[$i] >= 133 And $longitude[$i] < 139) {
        $term = SE_VENUS;
    }
    if ($longitude[$i] >= 139 And $longitude[$i] < 145) {
        $term = SE_JUPITER;
    }
    if ($longitude[$i] >= 145 And $longitude[$i] < 150) {
        $term = SE_MARS;
    }
    if ($longitude[$i] >= 150 And $longitude[$i] < 157) {
        $term = SE_MERCURY;
    }
    if ($longitude[$i] >= 157 And $longitude[$i] < 163) {
        $term = SE_VENUS;
    }
    if ($longitude[$i] >= 163 And $longitude[$i] < 168) {
        $term = SE_JUPITER;
    }
    if ($longitude[$i] >= 168 And $longitude[$i] < 174) {
        $term = SE_SATURN;
    }
    if ($longitude[$i] >= 174 And $longitude[$i] < 180) {
        $term = SE_MARS;
    }
    if ($longitude[$i] >= 180 And $longitude[$i] < 186) {
        $term = SE_SATURN;
    }
    if ($longitude[$i] >= 186 And $longitude[$i] < 191) {
        $term = SE_VENUS;
    }
    if ($longitude[$i] >= 191 And $longitude[$i] < 199) {
        $term = SE_JUPITER;
    }
    if ($longitude[$i] >= 199 And $longitude[$i] < 204) {
        $term = SE_MERCURY;
    }
    if ($longitude[$i] >= 204 And $longitude[$i] < 210) {
        $term = SE_MARS;
    }
    if ($longitude[$i] >= 210 And $longitude[$i] < 216) {
        $term = SE_MARS;
    }
    if ($longitude[$i] >= 216 And $longitude[$i] < 224) {
        $term = SE_JUPITER;
    }
    if ($longitude[$i] >= 224 And $longitude[$i] < 231) {
        $term = SE_VENUS;
    }
    if ($longitude[$i] >= 231 And $longitude[$i] < 237) {
        $term = SE_MERCURY;
    }
    if ($longitude[$i] >= 237 And $longitude[$i] < 240) {
        $term = SE_SATURN;
    }
    if ($longitude[$i] >= 240 And $longitude[$i] < 248) {
        $term = SE_JUPITER;
    }
    if ($longitude[$i] >= 248 And $longitude[$i] < 254) {
        $term = SE_VENUS;
    }
    if ($longitude[$i] >= 254 And $longitude[$i] < 259) {
        $term = SE_MERCURY;
    }
    if ($longitude[$i] >= 259 And $longitude[$i] < 265) {
        $term = SE_SATURN;
    }
    if ($longitude[$i] >= 265 And $longitude[$i] < 270) {
        $term = SE_MARS;
    }
    if ($longitude[$i] >= 270 And $longitude[$i] < 276) {
        $term = SE_VENUS;
    }
    if ($longitude[$i] >= 276 And $longitude[$i] < 282) {
        $term = SE_MERCURY;
    }
    if ($longitude[$i] >= 282 And $longitude[$i] < 289) {
        $term = SE_JUPITER;
    }
    if ($longitude[$i] >= 289 And $longitude[$i] < 295) {
        $term = SE_MARS;
    }
    if ($longitude[$i] >= 295 And $longitude[$i] < 300) {
        $term = SE_SATURN;
    }
    if ($longitude[$i] >= 300 And $longitude[$i] < 306) {
        $term = SE_SATURN;
    }
    if ($longitude[$i] >= 306 And $longitude[$i] < 312) {
        $term = SE_MERCURY;
    }
    if ($longitude[$i] >= 312 And $longitude[$i] < 320) {
        $term = SE_VENUS;
    }
    if ($longitude[$i] >= 320 And $longitude[$i] < 325) {
        $term = SE_JUPITER;
    }
    if ($longitude[$i] >= 325 And $longitude[$i] < 330) {
        $term = SE_MARS;
    }
    if ($longitude[$i] >= 330 And $longitude[$i] < 338) {
        $term = SE_VENUS;
    }
    if ($longitude[$i] >= 338 And $longitude[$i] < 344) {
        $term = SE_JUPITER;
    }
    if ($longitude[$i] >= 344 And $longitude[$i] < 350) {
        $term = SE_MERCURY;
    }
    if ($longitude[$i] >= 350 And $longitude[$i] < 356) {
        $term = SE_MARS;
    }
    if ($longitude[$i] >= 356 And $longitude[$i] < 360) {
        $term = SE_SATURN;
    }
...
//speed literal
    ...
    if ($i == SE_SUN And abs($speed[$i]) > .9856103) {
        $speed_literal = "Fast";
    }
    if ($i == SE_MERCURY And abs($speed[$i]) > 1.66667) {
        $speed_literal = "Fast";
    }
    if ($i == SE_VENUS And abs($speed[$i]) > 1.166667) {
        $speed_literal = "Fast";
    }
    if ($i == SE_MARS And abs($speed[$i]) > .716667) {
        $speed_literal = "Fast";
    }
    if ($i == SE_JUPITER And abs($speed[$i]) > .2000) {
        $speed_literal = "Fast";
    }
    if ($i == SE_SATURN And abs($speed[$i]) > .1000) {
        $speed_literal = "Fast";
    }
    if ($i == SE_MOON And abs($speed[$i]) > 14.166667) {
        $speed_literal = "Fast";
    }
    ...
    if ($i == SE_MERCURY And abs($speed[$i]) < .333333) {
        $speed_literal = "Slow";
    }
    if ($i == SE_VENUS And abs($speed[$i]) < .25000) {
        $speed_literal = "Slow";
    }
    if ($i == SE_MARS And abs($speed[$i]) < .166667) {
        $speed_literal = "Slow";
    }
    if ($i == SE_JUPITER And abs($speed[$i]) < .05000) {
        $speed_literal = "Slow";
    }
    if ($i == SE_SATURN And abs($speed[$i]) < .033333) {
        $speed_literal = "Slow";
    }
    if ($i == SE_MOON And abs($speed[$i]) < 12.2500) {
        $speed_literal = "Slow";
    }

    if (abs($speed[$i]) < .00833333) {
        $speed_literal = "stat";
    }

    if ($i == SE_TNNODE Or $i == SE_TSNODE) {
        $speed_literal = "";
    }
...
//peregrine
    ...
    if ($peregrine[$i] == 1 And $i <= SE_SATURN) {
        $peregrineX = "X";
    }
...
for ($i = SE_SUN; $i <= SE_SATURN; $i++) {
    ...
    if ($t == 3 Or $t == 5 Or $t == 8 Or $t == 9 Or $t == 11) {
        continue;
    }
