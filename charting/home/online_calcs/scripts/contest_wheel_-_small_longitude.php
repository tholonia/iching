...
//var_dump($_SESSION);
$sessionName = "contests_wheel_small_longitude";
...
// create the blank image
  ...
  $im = @imagecreatetruecolor($overall_size + $extra_width_for_graphic_data_table, $overall_size + $y_top_margin + $extra_height_for_graphic_data_table) or die("Cannot initialize new G['ID'] image stream");
...
// MUST BE HERE - ['ID'] DO NOT KNOW WHY - MAYBE TO PRIME THE PUMP
...
//shade the areas of complete signs, alternating colors - do not move this code from here
  for ($i = 0; $i <= 330; $i = $i + 30)
  {
    ...
    if ($flag == True)
    {
//      imagefilledarc($im, $center_pt_x, $center_pt_y, $diameter - ($inner_diameter_offset * 2) - 1, $diameter - ($inner_diameter_offset * 2) - 1, $angle, $angle + 30, $light_blue, ['ID']MG_ARC_PIE);
    }
    else
    {
//      imagefilledarc($im, $center_pt_x, $center_pt_y, $diameter - ($inner_diameter_offset * 2) - 1, $diameter - ($inner_diameter_offset * 2) - 1, $angle, $angle + 30, $light_blue, ['ID']MG_ARC_PIE);
...
// draw the image in png format - using imagepng() results in clearer text compared with imagejpeg()
  $hash_x = (isset($_SESSION['hash_x'])?$_SESSION['hash_x']:"");
